...
class DashboardViewModelTest {
    ...
    fun `uiState aggregates totals correctly`() = runTest {
        ...
        every { userPrefs.isFirstLaunch } returns flowOf(false)
    ...
    fun `uiState calculates chart data correctly`() = runTest {
        ...
        viewModel.uiState.test {
            ...
            assertEquals(1f, state.dashboard.totalChartData.last()) // Max is 100, so 100/100 = 1.0
            assertEquals(0.5f, state.dashboard.totalChartData[state.dashboard.totalChartData.size - 2]) // 50/100 = 0.5
        }
    }
}
